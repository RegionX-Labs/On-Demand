    ...
    T::AccountId: From<[u8; 32]>,
{
    pub fn new(keystore: KeystorePtr) -> Self {
        ...
        let binding = <SpMultiSignature as Verify>::Signer::from(pub_key)
            .into_account()
            .clone();
        ...
        let acc = T::AccountId::try_from(r)
            .ok()
            .expect("Failed to get account from keystore");
        Self {
    ...
    T::Signature: From<Signature>,
{
    ...
    fn sign(&self, signer_payload: &[u8]) -> T::Signature {
        let pub_key = self
            .keystore
            .sr25519_public_keys(sp_consensus_aura::sr25519::AuthorityPair::ID)[0];
...
{
    let queue_status_storage = relay_chain
        .get_storage_by_key(hash, QUEUE_STATUS)
        .await
        .ok()?;
    ...
    let active_config_storage = relay_chain
        .get_storage_by_key(hash, ACTIVE_CONFIG)
        .await
        .ok()?;
    ...
    if queue_status.is_some() && active_config.is_some() {
        let traffic = queue_status
            .expect("We ensured spot_traffic is Some above; qed")
            .traffic;
