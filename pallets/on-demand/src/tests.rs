...
fn set_slot_width_works() {
    new_test_ext().execute_with(|| {
        ...
        // Failure: Bad origin
        assert_noop!(
            OnDemand::set_slot_width(RuntimeOrigin::signed(1), 1),
            BadOrigin
        );
...
fn set_threshold_parameter_works() {
    new_test_ext().execute_with(|| {
        ...
        // Failure: Bad origin
        assert_noop!(
            OnDemand::set_threshold_parameter(RuntimeOrigin::signed(1), 1_000),
            BadOrigin
        );

        // Should be working fine
        assert_ok!(OnDemand::set_threshold_parameter(
            RuntimeOrigin::root(),
            1_000
        ));
